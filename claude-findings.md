...
### 1. ~~Device tag leaks encryption key material~~ — FIXED (HKDF with separate info labels)
### 2. ~~SHA-256 used as KDF with no domain separation~~ — FIXED (HKDF with separate info labels)
...
### 5. `minSdk = 35` should be 31 — blocks installation on Android 12-14
**`android/app/build.gradle.kts:12`**

`project.md` says min SDK 31, but `build.gradle.kts` has `minSdk = 35` (Android 15). This prevents installation on ~95% of Android devices. Almost certainly a typo where `minSdk` was set to the `targetSdk` value.
...
- **`macos/.../Models/ClipboardContent.swift`** — zero references
- **`android/.../models/ClipboardContent.kt`** — zero references
...
### 33. `project.md` says minSdk 31, `build.gradle.kts` has 35
Already noted as bug #5.
...
### 38. `findDevice(byTag:)` does keychain I/O + SHA-256 per device on every BLE advertisement
**`macos/.../Pairing/PairingManager.swift:63-68`**

Called on every BLE scan result (100-500ms intervals). Does a full keychain read + JSON decode + N SHA-256 hashes. Tags should be cached in memory.
